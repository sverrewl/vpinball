                ...
                UINT cNames, LCID lcid, DISPID *rgDispId)
{
	...
	*rgDispId = DISPID_UNKNOWN;
	...
	if (!wcsicmp(*rgszNames, L"Add")) {
		*rgDispId = 0x00000001;
	}
	else if (!wcsicmp(*rgszNames, L"Count")) {
		*rgDispId = 0x00000002;
	}
	else if (!wcsicmp(*rgszNames, L"Exists")) {
		*rgDispId = 0x00000003;
	}
	else if (!wcsicmp(*rgszNames, L"Items")) {
		*rgDispId = 0x00000004;
	}
	else if (!wcsicmp(*rgszNames, L"Key")) {
		*rgDispId = 0x00000005;
	}
	else if (!wcsicmp(*rgszNames, L"Keys")) {
		*rgDispId = 0x00000006;
	}
	else if (!wcsicmp(*rgszNames, L"Remove")) {
		*rgDispId = 0x00000007;
	}
	else if (!wcsicmp(*rgszNames, L"RemoveAll")) {
		*rgDispId = 0x00000008;
	}
	else if (!wcsicmp(*rgszNames, L"CompareMode")) {
		*rgDispId = 0x00000009;
	}
	else if (!wcsicmp(*rgszNames, L"HashVal")) {
		*rgDispId = 0x0000000a;
	}
                                      ...
                                      EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	switch(dispIdMember) {
		case DISPID_VALUE: {
			if (wFlags & DISPATCH_PROPERTYPUTREF) {
				...
				VARIANT var1;
				V_VT(&var1) = VT_EMPTY;
				VariantCopy(&var1, &pDispParams->rgvarg[--index]);
				hr = dictionary_putref_Item(iface, &var0, &var1);
				VariantClear(&var0);
				VariantClear(&var1);
			}
				...
				V_VT(&var1) = VT_EMPTY;
				VariantCopy(&var1, &pDispParams->rgvarg[--index]);
				hr = dictionary_put_Item(iface, &var0, &var1);
				VariantClear(&var0);
				VariantClear(&var1);
			}
				...
				V_VT(&var0) = VT_EMPTY;
				VariantCopy(&var0, &pDispParams->rgvarg[--index]);
				hr = dictionary_get_Item(iface, &var0, pVarResult);
				VariantClear(&var0);
			}
			break;
		}
		case 0x00000001: {
			if (wFlags & DISPATCH_METHOD) {
				...
				V_VT(&var1) = VT_EMPTY;
				VariantCopy(&var1, &pDispParams->rgvarg[--index]);
				hr = dictionary_Add(iface, &var0, &var1);
				VariantClear(&var0);
				VariantClear(&var1);
			}
			break;
		...
		case 0x00000002: {
			if (wFlags & DISPATCH_PROPERTYGET) {
				...
				hr = dictionary_get_Count(iface, (LONG*)&V_I4(pVarResult));
			}
			break;
		}
		case 0x00000003: {
			if (wFlags & DISPATCH_METHOD) {
				...
				hr = dictionary_Exists(iface, &var0, &V_BOOL(pVarResult));
				VariantClear(&var0);
			}
			break;
		}
		case 0x00000004: {
			if (wFlags & DISPATCH_METHOD) {
				...
				hr = dictionary_Items(iface, pVarResult);
			}
			break;
		}
		case 0x00000005: {
			if (wFlags & DISPATCH_PROPERTYPUT) {
				...
				V_VT(&var1) = VT_EMPTY;
				VariantCopy(&var1, &pDispParams->rgvarg[--index]);
				hr = dictionary_put_Key(iface, &var0, &var1);
				VariantClear(&var0);
				VariantClear(&var1);
			}
			break;
		...
		case 0x00000006: {
			if (wFlags & DISPATCH_METHOD) {
				...
				hr = dictionary_Keys(iface, pVarResult);
			}
			break;
		}
		case 0x00000007: {
			if (wFlags & DISPATCH_METHOD) {
				...
				V_VT(&var0) = VT_EMPTY;
				VariantCopy(&var0, &pDispParams->rgvarg[--index]);
				hr = dictionary_Remove(iface, &var0);
				VariantClear(&var0);
			}
			break;
		}
		...
		case 0x00000009: {
			if (wFlags & DISPATCH_PROPERTYPUT) {
				...
				hr = dictionary_put_CompareMode(iface, (CompareMethod)V_I4(&var0));
				VariantClear(&var0);
			}
			else if (wFlags & DISPATCH_PROPERTYGET) {
				...
				V_VT(pVarResult) = VT_I4;
				hr = dictionary_get_CompareMode(iface, (CompareMethod*)&V_I4(pVarResult));
			}
			break;
		}
		case DISPID_NEWENUM: {
			if (wFlags & DISPATCH_METHOD) {
				...
				hr = dictionary__NewEnum(iface, &V_UNKNOWN(pVarResult));
			}
			break;
		}
		case 0x0000000a: {
			if (wFlags & DISPATCH_PROPERTYGET) {
				...
				V_VT(&var0) = VT_EMPTY;
				VariantCopy(&var0, &pDispParams->rgvarg[--index]);
				hr = dictionary_get_HashVal(iface, &var0, pVarResult);
				VariantClear(&var0);
			}
			break;
		}
	...
	if (hr != S_OK) {
		if (dispIdMember == DISPID_VALUE && wFlags == (DISPATCH_METHOD | DISPATCH_PROPERTYGET)) {
			...
			printf("dictionary_Invoke, dispId=%d, wFlags=%d, hr=%d\n", dispIdMember, wFlags, hr);
		}
	}
	return hr;
#endif
